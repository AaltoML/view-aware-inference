...
parser.add_option(
    "--xdim", dest="xdim", type=int, default=128, help="rank of object linear covariance"
)
...
def main():
    ...
    vm = Vmodel(P, opt.xdim, poses).cuda()
    gp = GP(n_rand_effs=1)
    gp = torch.nn.DataParallel(gp, device_ids=[0, 1]).cuda()
    ...
    for epoch in range(opt.epochs):
        ...
        Zb, Vbs, vbs, gp_nll = gp.module.taylor_coeff(Z, [Vt])
        ...
        smartAppend(history, "vs", gp.module.get_vs().data.cpu().numpy())
        ...
        if epoch % opt.epoch_cb == 0:
            ...
            vae_wfile = os.path.join(wdir, "vae_weights.%.5d.pt" % epoch)
            torch.save(vae.state_dict(), vae_wfile)
            gp_wfile = os.path.join(wdir, "gp_weights.%.5d.pt" % epoch)
            torch.save(gp.state_dict(), gp_wfile)
            vm_wfile = os.path.join(wdir, "vm_weights.%.5d.pt" % epoch)
            torch.save(vm.state_dict(), vm_wfile)
...
def eval_step(vae, gp, vm, val_queue, Zm, Vt, Vv):
    ...
    with torch.no_grad():
        ...
        rv["vars"] = gp.module.get_vs().data.cpu().numpy()
        # out of sample
        vs = gp.module.get_vs()
...
def backprop_and_update(
    ...
    for batch_i, data in enumerate(train_queue):
        ...
        # forward gp
        _Vs = [vm(_d, _w)]
        gp_nll_fo = gp.module.taylor_expansion(z, _Vs, _Zb, _Vbs, vbs) / vae.K
